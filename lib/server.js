...
Server.startServer = function(server) {
  ...
  const respondWithError = function(responseMessage) {
    eventLog.logMessage(config, responseMessage, eventLog.LOG_INFO);
    if (Server.config.mqtt.responseTopic) {
      mqttClient.publish(Server.config.mqtt.responseTopic, responseMessage);
    }
  }
  ...
  mqttClient.on('message', function(topic, message) {
    if (topic === Server.config.mqtt.requestTopic) {
      if (Server.config.connections[message]) {
        if (Server.config.mqtt.responseTopic) {
          mqttClient.publish(Server.config.mqtt.responseTopic, 'calling');
        }
        ...
        if (dest !== undefined) {
          ...
          respondWithError('incorrect configuration for:' + message);
        }
      } else {
        respondWithError('cannot find:' + message);
      }
